...
/// Component for sum_reduce operations, using `SimdBackend` with fallback to `CpuBackend` for small traces.
pub type SumReduceComponent = FrameworkComponent<SumReduceEval>;

/// Defines the AIR for the sum_reduce component.
...
impl FrameworkEval for SumReduceEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let input_id = eval.next_trace_mask(); // ID of first input tensor.
        ...
        let acc = eval.next_trace_mask(); // Value in output tensor at index.
        let next_acc = eval.next_trace_mask(); // Value in output tensor at index.
        let is_last_step = eval.next_trace_mask(); // Value in output tensor at index.
        ...
        eval.add_constraint(next_acc.clone() - (acc.clone() + input_val.clone()));
        eval.add_constraint((out_val.clone() - next_acc) * is_last_step);
