...
Therefore, our work with LuminAIR involves harnessing Luminal’s composable compilers to develop efficient compilers 
specifically tailored for proving computational graphs using a ZK framework.
...
We are leveraging Stwo’s evaluation framework to define constraints for our AIR. 
LuminAIR employs a compiler that replaces each operation in the graph with its corresponding component in our AIR, 
